...
Complete reference for the Codex skill copies in this configuration. Each
skill is a `SKILL.md` prompt file that Codex can load when the task matches
the skill description or when the user names the skill explicitly.
...
Read the branch-specific state file, recent decisions, session logs, and lessons learned. Gathers the current branch, state file, lessons, decision/research listings, and git status, then outputs a brief summary of where you left off, what decisions are in effect, and recommended next steps. Run at the start of every session.
...
Save the current branch's state to `.planning/STATE-<branch-slug>.md`. Codex should use this proactively when decisions are made, dead ends are hit, scope changes, or sessions wrap up. State files stay under 80 lines.
...
Critical self-review of the current branch's changes (diff against main or master). Starts from the diff, builds an impact map of changed contracts and outside-diff consumers, and reads relevant unchanged call sites before judging correctness. For diffs touching 3+ files, reads [`agents.md`](review/agents.md) and spawns 6 parallel read-only analysis agents: correctness & security, code quality, test quality, production readiness, documentation & maintenance, and architecture & responsibility boundaries. For smaller diffs, reads [`criteria.md`](review/criteria.md) and runs the criteria in a single pass. Auto-fixes critical and moderate findings; lists minor findings for the user to decide on. Iteration-aware: on follow-up runs, gives extra scrutiny to files touched by recent review-fix commits. Designed to be run 2-3 times iteratively.

Review criteria: correctness, security, readability, naming & conventions, type safety, pattern adherence, testability, scalability, observability, documentation & maintenance, and architecture & responsibility boundaries.
...
Compares documentation against actual code to find: dead file paths and command references, outdated setup instructions and architecture descriptions, missing docs for new features and env vars, and inconsistencies across README/AGENTS.md/CLAUDE.md/CONTRIBUTING.md. Fixes high and medium severity issues directly. Also integrated into `$ship` (targeted check) and `$review-project` (cross-cutting concerns).
