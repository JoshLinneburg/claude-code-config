...
the full diff, the impact map, relevant unchanged consumer files, and the
project's AGENTS.md/CLAUDE.md/CONTRIBUTING.md content. Each performs a
**read-only** analysis - no edits, no fixes. Each returns a list of findings.

The impact map is mandatory. It lists changed symbols/contracts and the
consumer files outside the diff that were checked. Do not limit review to
the changed files when a changed function, type, endpoint, schema, event,
config key, or serialized shape has callers or dependents elsewhere.
...
- **Outside-diff consumers** - For every changed callable, type, endpoint,
  payload shape, config key, or event in the impact map, read the listed
  unchanged consumers. Verify signatures, argument order, default values,
  return shapes, side effects, exceptions, async behavior, and lifecycle
  expectations still match. If a consumer should have been updated but was
  not in the diff, flag it.
...
- **Pattern Adherence** - Does the code follow the patterns
  established in this project (from AGENTS.md, CLAUDE.md, CONTRIBUTING.md, and
  ...
  **Actively compare against siblings**: for any new or changed file,
  ...
  is a pattern violation. Don't just check instruction files - check the
  actual code around it.
- **Consumer conventions** - For unchanged consumers in the impact map,
  verify the changed code still presents the API shape and conventions
  those consumers expect. A clean diff can still be wrong if it forces
  existing call sites into awkward adapters or inconsistent usage.
...
For each changed source file and each behaviorally relevant consumer file
from the impact map, find corresponding test file(s) using the project's
naming convention. If no test file exists, that's a finding.
...
- **Consumer coverage** - If a changed contract is consumed outside the
  diff, do tests cover at least one real consumer path? A unit test for the
  changed function alone may miss a broken caller.
...
- **Project documentation** - Do the changes affect behavior described in
  READMEs, `docs/` files, AGENTS.md, CLAUDE.md, or CONTRIBUTING.md? If so, are those
...
- **Contract documentation** - If the impact map finds public consumers of
  a changed contract, check whether the public docs, examples, generated
  clients, or README snippets describing that contract still match.
...
- **Impact radius** - Does the changed module's dependency surface still
  make sense once unchanged consumers are included? Check whether a
  local-looking diff actually changes a public boundary, forces callers
  to know internal details, or leaves downstream code using stale
  assumptions.
