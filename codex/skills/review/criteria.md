...
Evaluate against ALL 11 criteria and the impact-radius requirement. Do not
skim. Read the code carefully.

## Required Impact-Radius Pass

Start from the diff, but do not stop there:
- Identify changed exported functions/classes/types, component props, API
  endpoints, route handlers, event names, database schemas, config keys,
  env vars, CLI flags, serialized payload shapes, and other contracts.
- Search for consumers outside the diff with `rg`, import/reference
  searches, route/client lookups, and test references.
- Read relevant unchanged consumers and verify they still call the changed
  code correctly, handle returned values correctly, preserve invariants,
  and still satisfy authorization, validation, error-handling, and
  observability expectations.
- If a changed contract has consumers that were not updated in the diff,
  verify that no update is needed. If that cannot be proven, flag the risk.
- Include outside-diff findings with the consumer file and line number, not
...
- For changed callables, types, payloads, endpoints, config keys, and event
  names, do all known unchanged consumers still use the contract correctly?
...
- Does the code follow the patterns established in this project? Check
  the project's AGENTS.md, CLAUDE.md, CONTRIBUTING.md, and surrounding
  code for conventions.
...
- If a changed contract is consumed outside the diff, is at least one real
  consumer path tested?
...
- Are there unnecessary loops, redundant computations, or O(n^2)
...
- Do the changes affect behavior described in READMEs, `docs/` files,
  AGENTS.md, CLAUDE.md, or CONTRIBUTING.md? If so, are those docs still
  accurate?
