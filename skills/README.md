...
Read the branch-specific state file, recent decisions, session logs, and lessons learned. Outputs a brief summary of where you left off, what decisions are in effect, and recommended next steps. Run at the start of every session.
...
Compares documentation against actual code to find: dead file paths and command references, outdated setup instructions and architecture descriptions, missing docs for new features and env vars, and inconsistencies across README/CLAUDE.md/CONTRIBUTING.md. Fixes high and medium severity issues directly. Also integrated into `/ship` (targeted check) and `/review-project` (cross-cutting concerns).
...
Read-only cross-branch dashboard. Shows all local branches and worktrees with: last commit and age, uncommitted changes, one-line summary from each branch's `.planning/` state file, and CI status (if `gh` is available). Flags branches with no activity in 30+ days and stale state files.
