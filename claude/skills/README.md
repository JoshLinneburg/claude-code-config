...
Read the branch-specific state file, recent decisions, session logs, and lessons learned. Pre-loads branch name, git status, and local branches via shell injection, then uses explicit Read/Glob steps for state, lessons, ROADMAP, decisions, research, and sessions. Outputs a brief summary of where you left off, what decisions are in effect, and recommended next steps. Run at the start of every session.
...
Critical self-review of the current branch's changes (diff against main). Uses dynamic context injection to pre-load changed files, diff stats, and recent commits before analysis begins. Before analysis, runs a **Blast Radius Mapping** phase: identifies the exported surface touched by the diff, classifies each change as behavior-affecting or internal-only, and produces a Consumer Files list (callers, importers, dependent docs/configs). Every subsequent analysis step is scoped to the diff *plus* its consumers — a call site that compiles is not necessarily a call site that works. For diffs touching 3+ files, reads [`agents.md`](review/agents.md) and spawns 6 parallel read-only analysis agents — (1) correctness & security, (2) code quality, (3) test quality, (4) production readiness, (5) documentation & maintenance, (6) architecture & responsibility boundaries. For smaller diffs, reads [`criteria.md`](review/criteria.md) and runs all 11 criteria in a single pass. Auto-fixes critical and moderate findings; lists minor findings for the user to decide on. Iteration-aware: on follow-up runs, gives extra scrutiny to files touched by recent review-fix commits. Designed to be run 2-3 times iteratively.

Review criteria: correctness, security, readability, naming & conventions, type safety, pattern adherence, testability, scalability, observability, documentation & maintenance, architecture & responsibility boundaries.

> **Note:** Claude Code now ships a built-in `/review` slash command (PR-style review). This skill deliberately shadows it — the user-defined skill takes precedence on name resolution. Use `/josh-test` as an unambiguous alias.
