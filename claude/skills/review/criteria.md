...
The review scope is **the diff plus the Consumer Files list produced by
Blast Radius Mapping in SKILL.md**. Every criterion below applies to
both the changed files and the files that consume them. A behavior-
affecting change (signature, semantics, return shape, error behavior,
schema, rename) must be verified at every consumer site — "compiles" is
not "correct."
...
- **Consumer correctness** — For each behavior-affecting change in the
  diff, read each consumer site in the Consumer Files list and verify
  it still does the right thing: caller passes the right values to
  new/changed params; correctly handles a changed return shape (uses
  new fields, doesn't rely on removed ones, handles null/optional);
  catches any newly-raised exception or stops catching one that's no
  longer thrown; reflects changed side effects; updates every
  reference to a renamed/removed symbol; passes the new meaning of a
  semantically-changed constant. A consumer that compiles is not
